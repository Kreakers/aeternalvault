#!/usr/bin/env bash
# Claude Code Oyun Stüdyoları — Durum Satırı
# stdin'de JSON alır, tek satırlık durum çıkarır.
#
# Segmentler: ctx% | model | üretim aşaması [| Epic > Feature > Task]

input=$(cat)

# --- JSON'u ayrıştır (jq grep geri dönüş ile) ---
...
# Windows yollarını normalleştir
...
# --- Üretim aşaması ---
# Öncelik 1: stage.txt'den açık aşama
...
# Öncelik 2: Yapıtlardan otomatik algılama
if [ -z "$stage" ]; then
  ...
  # Motor yapılandırılıp yapılandırılmadığını kontrol et (yer tutucu değil)
  ...
  # Kaynak dosyalarını say (dilin bağımsız)
  if [ -d "$cwd/src" ]; then
  ...
  # Aşamayı belirle (en gelişmiş geri kontrol et)
...
if [ "$stage" = "Production" ] || [ "$stage" = "Polish" ] || [ "$stage" = "Release" ]; then
  ...
  if [ -f "$state_file" ]; then
    ...
    # Kırıntı ne set edilirse buildını yap
    parts=""
    [ -n "$epic" ] && parts="$epic"
